...
class Step(db.Model):
    ...
    def can_user_execute(self, user):
        """Check if user has required skills to execute this step"""
        if self.step_type:
            # Map step types to required skills (using actual skill names from database)
            skill_mapping = {
                'model_implementation': ['Modelowanie 3D', 'Inżynieria CAD', 'Wdrożeniowiec'],
                'sharepoint_confirmation': ['SharePoint', 'Dokumentacja techniczna'],
                'illustration_technologist': ['Ilustracje techniczne', 'Inżynieria CAD'],
                'illustration_designer': ['Ilustracje konstrukcyjne', 'Projektowanie'],
                'bom_preparation': ['Przygotowanie wyceny', 'Analiza BOM', 'Inżynieria CAD']
            }
            required_skills = skill_mapping.get(self.step_type, [])
            user_skills = user.get_skills_list()
            return any(skill in user_skills for skill in required_skills)
        return True  # If no step_type defined, any user can execute
