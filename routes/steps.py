...
def list_steps():
    ...
    users = User.query.all() if current_user.has_role('admin') else [current_user]
...
def view_step(step_id):
    ...
    if not current_user.has_role('admin') and step.assigned_to != current_user.id:
...
def edit_step(step_id):
    ...
    if not current_user.has_role('admin') and step.assigned_to != current_user.id and step.created_by != current_user.id:
...
def _save_step(step_id):
    ...
    if not name:
        ...
        # Validate assigned user has required roles for this step type
        if assigned_to and step_type:
            from app.models.user import User
            from app.models.role import Role
            assignee = User.query.get(assigned_to)
            if assignee:
                # Check if assignee has at least one of the step's required roles (if any)
                # The can_user_execute method already handles this check
                pass  # We'll let the step validation happen after creation
...
def _show_create_form():
    ...
    return render_template('steps/form.html', step=None, projects=projects, users=users, skills=skills, roles=roles, templates=templates)

@steps_bp.route('/export')
@login_required
def export_steps():
    """Eksportuj zadania do Excel"""
    from app.services.import_export import export_steps_to_excel
    try:
        ...
        output = export_steps_to_excel(query)
        return send_file(
            output,
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            as_attachment=True,
            download_name='zadania_kit.xlsx'
        )
    except Exception as e:
        flash(f'Błąd eksportu: {str(e)}', 'error')
        return redirect(url_for('steps.list_steps'))

@steps_bp.route('/import', methods=['GET', 'POST'])
@login_required
def import_steps():
    """Importuj zadania z Excela (admin only)"""
    if not current_user.has_role('admin'):
    ...
    if request.method == 'POST':
        if 'file' not in request.files:
            ...
            return redirect(url_for('steps.import_steps'))
        ...
        try:
            from app.services.import_export import import_steps_from_excel
            result = import_steps_from_excel(file.stream, current_user.id)
            
            if result['success'] or result['imported'] > 0 or result['updated'] > 0:
                if result['imported'] > 0:
                    flash(f'Zaimportowano {result["imported"]} nowych zadań.', 'success')
                if result['updated'] > 0:
                    flash(f'Zaktualizowano {result["updated"]} istniejących zadań.', 'info')
                if result['errors']:
    ...
    return render_template('steps/import.html')
...
def _show_edit_form(step_id):
    ...
    roles = Role.query.all()
    templates = StepTemplate.query.filter_by(is_active=True).order_by(StepTemplate.name).all()
    return render_template('steps/form.html', step=step, projects=projects, users=users, skills=skills, roles=roles, templates=templates)
...
def delete_step(step_id):
    ...
    if not current_user.has_role('admin') and step.created_by != current_user.id:
