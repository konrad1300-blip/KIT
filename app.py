...
Poprawiona wersja z obsługą Proxy i Gunicorna.
"""
...
from werkzeug.middleware.proxy_fix import ProxyFix  # Import dla obsługi Proxy
...
app = Flask(__name__)
app.secret_key = 'twoj_bardzo_tajny_klucz_kit' # Zmień na unikalny w produkcji

# --- KONFIGURACJA PROXY (Apache) ---
# Informuje Flaska, że działa za Proxy. To naprawia błędy z adresami URL i CSS.
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1, x_host=1, x_prefix=1)

# --- KONFIGURACJA ŚCIEŻEK ---
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
DATABASE = os.path.join(BASE_DIR, 'kit.db')
UPLOAD_FOLDER = os.path.join(BASE_DIR, 'uploads')
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB max

# Automatyczne tworzenie folderu na pliki
...
def upload_file(task_id):
    ...
    filename = f"{task_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{original_filename}"
    filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
...
def complete_step(task_id, step_id):
    ...
    # Logika statusu:
    # - Jeśli wszystkie kroki ukończone -> "zakończone" (zielony)
...
def suggested_performers(step_id):
    ...
    # Znajdź użytkowników z odpowiednią rolą (obsługa wielu ról oddzielonych przecinkami)
    role_param = step['role_required']
    required_roles = [r.strip() for r in role_param.split(',')]
    
    # Budujemy zapytanie z IN dla wielu ról
    placeholders = ','.join('?' for _ in required_roles)
    query = f"SELECT * FROM users WHERE role IN ({placeholders}) OR role = 'admin'"
    params = required_roles
    
    potential_users = db.execute(query, params).fetchall()
...
# Inicjalizacja bazy danych przy ładowaniu modułu (ważne dla Gunicorna)
init_db()
...
if __name__ == '__main__':
    ...
    Uruchamia serwer Flask development server.
